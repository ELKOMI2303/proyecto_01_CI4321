...
class GameScene { 
  ...
  private onKeyDown = (event: KeyboardEvent) => { 
    ...
    switch (event.key.toLowerCase()) { 
      ...
      case "3": // Cambiar entre cámaras
        ...
        console.log(`Camera Mode: ${this.isThirdPerson ? 'Third Person' : 'First Person'}`);
        break;
    } 
  }; 
  ...
  private updateCameraPosition() {
    ...
    const cameraPosition = vehiclePosition.clone().add(cameraOffset.applyQuaternion(this._vehicle.group.quaternion));
  ...
  private checkCollisions() {
    ...
    projectiles.forEach((projectile) => {
        ...
        const projectileRadius = projectile.mesh.geometry.boundingSphere?.radius || 0.5; // Radio del proyectil
        ...
        const distanceToCube = this.distanceBetween(projectile.mesh.position, cube.position);

        // Calcula la distancia entre el proyectil y la pirámide
        const distanceToPyramid = this.distanceBetween(projectile.mesh.position, pyramid.position);

        // Verifica si la distancia es menor o igual a la suma de los radios del cubo
        if (projectile.active && distanceToCube <= combinedRadiusCube && this.cubeLives > 0) {
        ...
        } else if (projectile.active && distanceToPyramid <= combinedRadiusPyramid && this.pyramidLives > 0) {
