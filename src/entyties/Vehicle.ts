...
class Vehicle {
  ...
  constructor() {
    ...
    // Crear el material (opcionalmente, cambia el material si es necesario)
    ...
    wheelGeometry.setAttribute('position', new BufferAttribute(new Float32Array(wheelVertices), 3));
    ...
    centerGeometry.setAttribute('position', new BufferAttribute(centerVertices, 3));
    centerGeometry.setIndex(centerIndices);

    // Materiales
    const wheelMaterial = new MeshBasicMaterial({ color: 0x333333, side: DoubleSide });
    wheelMaterial.depthWrite = true;

    const centerMaterial = new MeshBasicMaterial({ color: 0xff0000, side: DoubleSide });
    ...
    for (let y = 0; y <= heightSegments; y++) {
      ...
      for (let x = 0; x <= widthSegments; x++) {
        ...
        // Calcular normales
        normals_esphere.push(vertexX / radius, vertexY / radius, vertexZ / radius);
      }
    }
  ...
  this.cannonSphereContainer.position.set(0, 0, -3); // Posición del contenedor
    ...
    const cannonBarrelVertices = new Float32Array([
      // Frente (z positivo)
      -0.25, -0.25, 1.5,  // Vértice 0
      0.25, -0.25, 1.5,  // Vértice 1
      0.25, 0.25, 1.5,  // Vértice 2
      -0.25, 0.25, 1.5,  // Vértice 3

      // Atrás (z negativo)
      -0.25, -0.25, -1.5, // Vértice 4
      0.25, -0.25, -1.5, // Vértice 5
      0.25, 0.25, -1.5, // Vértice 6
      -0.25, 0.25, -1.5  // Vértice 7
    ]);
    ...
    cannonBarrelGeometry.setAttribute('position', new BufferAttribute(cannonBarrelVertices, 3));
    cannonBarrelGeometry.setIndex(cannonBarrelIndices);
    cannonBarrelGeometry.computeVertexNormals();

    // Material del barril
    const cannonBarrelMaterial = new MeshBasicMaterial({ color: 0x000000, side:DoubleSide });
    ...
    this.muzzle.position.set(0, 0, 1.5); // Cambiado de -1.5 a 1.5 para coincidir con la dirección positiva
...
public rotateCannonYaw(angle: number) {
  ...
  this.cannonBarrelContainer.quaternion.multiplyQuaternions(quaternion, this.cannonBarrelContainer.quaternion);

  // Actualizar la flecha de dirección
  this.updateDirectionArrow();
}

// Rotación del cañón en pitch usando quaternions
public rotateCannonPitch(angle: number) {
  let newPitch = this.currentPitch + angle;
  newPitch = Math.max(this.minPitch, Math.min(newPitch, this.maxPitch));
...
public rotateLeft(delta: number) {
  ...
  this.group.quaternion.multiplyQuaternions(quaternion, this.group.quaternion);
...
public rotateRight(delta: number) {
  ...
  private updateDirectionArrow() {
    ...
    this.directionArrow.position.copy(this.muzzle.getWorldPosition(new Vector3()));
  }
  ...
  public update(delta: number) {
    ...
    this.direction.set(0, 0, 1).applyQuaternion(this.group.quaternion).normalize();
